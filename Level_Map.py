...
class Level_Map(pygame.Surface):
    ...
    def check_collide(self, pos: Tuple[int, int]):
        if self.rect.collidepoint(pos):
            print(self.tailing_coord((pos[0] + self.shift_x, pos[1] + self.shift_y)))

    def update(self):
        self.fill("violet")

        self.blit(pygame.Surface((32, 32)),
                  (32*-10 - self.shift_x, 32*-10 - self.shift_y))

        self.blit(pygame.Surface((32, 32)),
                  (32*0 - self.shift_x, 32*0 - self.shift_y))
